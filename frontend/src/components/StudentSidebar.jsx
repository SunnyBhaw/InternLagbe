...
const StudentSidebar = ({ user, isOpen, setIsOpen }) => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        console.log('Profile not found for sidebar display');
      }
    };
    fetchProfile();
  }, []);
  ...
  const menuItems = [
    ...
    { title: 'Browse Internships', path: '/student/browse', icon: Search },
    { title: 'My Applications', path: '/student/applications', icon: FileText },
    { title: 'Profile', path: '/student/profile', icon: User },
  ];

  // User details: Prefer Profile First/Last name, fallback to User name
