        ...
        <div className="flex h-screen bg-bg-secondary overflow-hidden font-sans">
                    ...
                    <div className="max-w-7xl mx-auto">
                        ...
                        <div className="bg-white p-4 rounded-3xl border border-border-light mb-8 flex flex-col md:flex-row md:items-center space-y-4 md:space-y-0 md:space-x-4 shadow-sm">
                            ...
                            <div className="flex items-center space-x-2 bg-bg-tertiary px-4 py-1.5 rounded-2xl border border-border-light">
                                ...
                                <select 
                                    className="bg-transparent border-none focus:ring-0 text-sm font-bold text-primary py-2 pr-8 cursor-pointer"
                                    value={statusFilter}
                        ...
                        {error ? (
                            <div className="bg-red-50 border border-red-100 p-12 rounded-[2.5rem] flex flex-col items-center text-center">
                                <AlertCircle size={48} className="text-red-500 mb-4" />
                                <h3 className="text-xl font-black text-red-600 mb-2">Error Loading Internships</h3>
                            ...
                            <div className="bg-white border border-border-light p-20 rounded-[2.5rem] flex flex-col items-center text-center shadow-sm">
                                ...
                                <h3 className="text-2xl font-black text-primary mb-2">No internships found</h3>
                                    ...
                                    <table className="w-full text-left">
                                        <thead>
                                            <tr className="bg-bg-tertiary/30 border-b border-border-light">
                                                ...
                                                <th className="px-8 py-6 text-xs font-black text-text-tertiary uppercase tracking-widest">Company</th>
                                                <th className="px-8 py-6 text-xs font-black text-text-tertiary uppercase tracking-widest">Category</th>
                                                <th className="px-8 py-6 text-xs font-black text-text-tertiary uppercase tracking-widest">Deadline</th>
                                                    ...
                                                    <tr key={ins._id} className="hover:bg-bg-tertiary/10 transition-colors group">
                                                        <td className="px-8 py-6">
                                                            <div className="flex flex-col">
                                                                <span className="font-bold text-primary group-hover:text-primary-light transition-colors">{ins.title}</span>
                                                                {(() => {
                                                                    const isExpired = new Date(ins.deadline) < new Date();
                                                                    const isClosed = ins.status === 'closed' || isExpired;
                                                                    
                                                                    return (
                                                                        <span className={`text-[10px] font-black uppercase tracking-widest mt-1 ${isClosed ? 'text-red-500' : 'text-emerald-500'}`}>
            ...
            {deleteId && (
                <div className="fixed inset-0 bg-primary/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white w-full max-w-md rounded-[2.5rem] p-10 shadow-2xl border border-border-light animate-in zoom-in-95 duration-200">
                        <div className="h-20 w-20 bg-red-50 text-red-500 rounded-3xl flex items-center justify-center mx-auto mb-8">
                            <Trash2 size={40} />
                        </div>
                        <h3 className="text-2xl font-black text-primary text-center mb-3 tracking-tight">Remove Internship?</h3>
